...
This document plans work only. Milestones 000 through 006C are **DONE**; their evidence is retained
in the corresponding completed ExecPlans. Before any milestone starts, its ExecPlan must be created
and maintained as required by `PLANS.md`. Milestone 006D is the next **NOT STARTED** milestone;
...
### Milestone 006D — Context Auto and First-Run Runtime Compatibility Hotfix

- **Status:** NOT STARTED.
- **Objective:** Correct context-window Auto semantics and make the M006C first-run runtime failure understandable without weakening runtime safety or changing later milestone boundaries.
- **Rationale for position:** M006C exposed the installed first-run path and revealed that Jarvis's 8192 default overrides llama-server's model-derived context contract. The same path also surfaced a real generic model-load incompatibility that currently collapses to an unhelpful `runtime.start_failed` result.
- **User-visible result:** Fresh/default model configuration displays Auto, persists `context_window = 0`, omits `--ctx-size`, and uses the effective context reported by a ready llama-server. Explicit context values remain unchanged and are emitted exactly. Setup accepts blank/0 as Auto, `/context` says Auto rather than “0 tokens,” and first-run failures show bounded generic reasons such as model incompatibility or timeout.
- **Exact scope:** Change `ModelRuntimeConfig` and packaged defaults to `0 = Auto` with explicit values bounded to 1,000,000; bump product defaults 5→6 while retaining defaults schema 5 and adding no SQL migration; preserve existing persisted explicit values; conditionally omit or emit `--ctx-size`; obtain Auto's effective context from bounded authenticated `GET /props` after readiness; make Context Builder consume effective runtime context; require the existing stop/update/start lifecycle for Auto↔explicit changes; add bounded allowlisted startup-failure classification and human-only setup rendering; verify current llama-server compatibility defaults without speculative changes.
- **Architectural components introduced or refined:** Provider/runtime effective-context readiness result, Auto-aware Context Builder handoff, setup error projection/presentation, and runtime configuration transition enforcement. No new host capability or client-owned authority is introduced.
- **Important interfaces/contracts introduced:** `context_window` wire/domain contract (`0` Auto, `1..1_000_000` explicit); product-default version 6 transition preserving stored values; conditional argv contract; bounded `/props` effective-context contract; allowlisted startup reason classes; human-only safe setup error presentation with raw provider output excluded from IPC and model context.
- **Persistence/database implications:** Existing `profile_models.runtime_config_json` remains authoritative; no schema or SQL migration is added. Fresh associations and reset-then-reselect use Auto. Existing nonzero JSON values are not rewritten.
- **Security implications:** Preserve descriptor-bound model/executable identity, authenticated loopback health, listener/process validation, timeouts, cleanup, centralized redaction, profile/model isolation, and diagnostic/model-context separation. Never expose raw stderr, paths, tensor names, argv, secrets, prompts, or model output.
- **Tests required:** Domain/default/version/wire/reset compatibility; exact Auto/explicit argv; bounded `/props`; effective-context prompt budgeting; setup blank/0/numeric behavior; Auto↔explicit lifecycle; active configuration divergence prevention; bounded startup reason classes; reported incompatible-GGUF failure; compatible local-GGUF Auto readiness when available; all predecessor, marker, lint, type, wheel, installed-M006C, and diff checks.
- **Dependencies:** Completed Milestones 000–006C, especially M004 model persistence/discovery, M005 provider/runtime ownership, M006A Context Builder/runtime coordination, M006B CLI presentation, and M006C setup/installation activation.
- **Explicit out of scope:** M007+, tools, Policy Engine, Tool Broker, web, memory, aliases, updater, autostart, TUI, downloads, hardware/VRAM estimation, model-specific exceptions, broad “all runtime settings Auto” redesign, and any weakening of health/process/security validation.
- **Manual verification:** In disposable installed and development XDG roots, exercise fresh Auto setup, inspect exact provider argv, complete a compatible local-GGUF Auto chat where available, reproduce the reported GGUF as a generic model-load incompatibility, verify explicit context remains explicit, and confirm no raw provider output crosses the human/model boundary.
- **Definition of done:** Auto semantics, persistence compatibility, setup UX, effective-context budgeting, runtime transition behavior, bounded startup diagnostics, and the complete predecessor/installed acceptance matrix pass without starting M007 or changing completed historical ExecPlans.
