...
This document plans work only. Milestone 000 is implemented but remains **IN PROGRESS** pending its
required CPython 3.12 verification; Milestone 001 has not started and is not authorized. Before any
milestone starts, its self-contained ExecPlan must be created and maintained as required by
`PLANS.md`.
...
4. The first chat path is split into a Core pipeline and a simple client slice. The Core slice owns Agent Engine, Context Builder, persistence, diagnostics, FIFO generation scheduling and first-chat learning activation; the client slice makes those contracts visibly usable without implementing a second brain.
...
- **Dependencies:** Milestones 002, 006B, 008, 013B, and 015.
...
- **Rationale for position:** Single-Core ownership, runtime recovery and management diagnostics are mature.
...
- **Exact scope:** One systemd-user Core unit; profile-owned desired autostart state; Core-side reconciliation or thin IPC activation; last-valid-model/no-substitution behavior; login/logout/restart recovery; status/repair integration.
- **Architectural components introduced:** systemd-user integration and autostart reconciler, never a second runtime manager.
- **Important interfaces/contracts introduced:** Exactly-one-Core topology; idempotent desired-state reconciliation; last-valid-model failure result; runtime startup does not consume first-run learning state.
...
- **Tests required:** Unit generation/idempotency; simultaneous/duplicate login activation; single-Core lock; multiple autostart profiles under one owner; missing model; stale runtime/PID reuse; logout/restart; learning non-activation; no-root paths.
- **Dependencies:** Milestones 005 and 018A.
- **Explicit out of scope:** Update application, installer/uninstaller, purge, machine-wide services.
...
- **Definition of done:** Reproducible user-local packages install and uninstall safely while preserving the complete in-scope product and protected-install identity.
...
- **Rationale for position:** The installed manifest/trust boundary from 019A and every migration/data-preservation contract already exist.
