...
Progress log: 2026-08-21 America/Sao_Paulo — Independent adversarial review found and fixed four narrow defects: (1) the M003 reset IPC route accepted standalone `startup` scope, exposing the M018B-owned autostart preference despite the M003 contract; it now rejects that scope while retaining whole-profile reset. (2) terminal rendering escaped C0/C1 controls but not Unicode bidirectional format controls; it now renders all `Cf` characters visibly. (3) escaped lone-surrogate JSON could raise raw `UnicodeEncodeError` during framing-tree bounds validation; it is now translated to the typed `ipc.invalid_frame` rejection. (4) `jarvis-config` recognized only unsequenced pre-acceptance IPC errors, allowing accepted terminal error events to become raw renderer exceptions; it now treats both protocol forms as typed client errors. Regressions added for all four. Final full verification is pending this review.

Progress log: 2026-08-21 America/Sao_Paulo — Independent review final evidence: CPython 3.12.13 and 3.14.4 each pass unit 183, integration 85, migration 36, security 62, and full 366; CPython 3.13 is unavailable. The committed M000–M002 predecessor selection passes 345 on both. Focused M003 IPC/security/terminal regressions, Ruff check, Ruff format check, strict mypy, and `git diff --check` pass. A fresh wheel clean-installs under CPython 3.12 with `pip check`, packaged defaults/migrations, dev-extra-only metadata requirements, and exactly `jarvisd`, `jarvis-config`, and `jarvis-help` scripts. The final disposable-XDG walkthrough passed local help before Core startup, current-Jarvis cloning, Unicode alias lifecycle, revision-checked configuration/reset/delete behavior, Core restart/persistence, an unrelated `work` PATH command, no alias artifacts/symlinks/PATH mutation, and clean shutdown.
...
Independent review correction (2026-08-21 America/Sao_Paulo): standalone `startup` reset was removed from the M003 IPC surface because it contradicted the documented M018B ownership of autostart. This is a scope-conformance correction, not a product decision or persistence change.
...
**Independent review update (2026-08-21 America/Sao_Paulo): DONE.** After four fixes, CPython 3.12.13 and 3.14.4 matrices are unit 183, integration 85, migration 36, security 62, full 366; the committed predecessor selection is 345 on both. The fresh wheel and disposable-XDG evidence above were repeated after the final production/test change. Schema remains 2, packaged migrations remain exactly 0001/0002, defaults remain 2/2, and no physical alias/PATH/M004+ behavior was introduced.
