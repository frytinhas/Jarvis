...
def _payload_reset_scope(payload: Mapping[str, object]) -> ResetScope:
    ...
    # Autostart is intentionally retained as M001 state only.  M003 may reset
    # it as part of an explicit whole-profile reset, but must not expose a
    # standalone autostart-management route before M018B owns that feature.
    if scope is ResetScope.STARTUP:
        raise ipc_error("ipc.operation_not_supported", operation="profiles.reset.preview")
    return scope
